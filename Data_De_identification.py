...
def add_column_with_button(column):
    ...
    method_menu = ttk.Combobox(frame, state="readonly", values=[
        "Replace with **", "Replace with ***", "SHA-256 Encrypt", "Mask Name",  # Mask Name 추가
        "Mask Phone", "Categorize Age", "Mask Address", "Round Up Square Footage", "Round Up Monthly Payment"], width=30)
...
# 이름의 앞글자만 남기고 나머지를 *로 변환하는 함수
def mask_name(name):
    if pd.notna(name) and isinstance(name, str) and len(name) > 1:
